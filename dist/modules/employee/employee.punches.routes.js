...
function endOfDayExclusive(dateStr) {
...
function computeDailyBuckets(total, cfg) {
    let reg = total, ot = 0, dt = 0;
    if (!cfg.overtimeDailyEnabled)
        return { reg, ot, dt };
...
function buildDayPairs(punches) {
...
async function getEmployeeWithLocation(orgId, employeeId) {
...
/* ---------------------------------------------------------
   ROUTES
--------------------------------------------------------- */
router.get("/:id/punches", verifyToken_1.verifyToken, async (req, res) => {
    try {
...
/* ---------------------------------------------------------
   GET TIME CARD
--------------------------------------------------------- */
router.get("/:id/timecard", verifyToken_1.verifyToken, async (req, res) => {
    try {
        const orgId = req.user?.organizationId;
        const employeeId = Number(req.params.id);
        if (!orgId || isNaN(employeeId)) {
        ...
        const startDateParam = typeof req.query.startDate === "string"
            ? req.query.startDate
            : undefined;
        const endDateParam = typeof req.query.endDate === "string"
            ? req.query.endDate
            : undefined;
        const startStr = parseDateOnly(startDateParam) ??
            addDays(formatDateOnly(new Date()), -13);
        const endStr = parseDateOnly(endDateParam) ?? formatDateOnly(new Date());
        const locked = await (0, payperiod_routes_1.isDateLockedForOrg)(employee.organizationId, employee.locationId, new Date(`${startStr}T12:00:00Z`));
        const punches = await prisma_1.prisma.punch.findMany({
            where: {
                employeeId,
                timestamp: {
