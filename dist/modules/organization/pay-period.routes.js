 ...
 * =====================================================
 */
router.patch("/", verifyToken_1.verifyToken, async (req, res) => {
    try {
        const { organizationId } = req.user;
        const { payPeriodType, weekStartDay, biweeklyAnchorDate, cutoffTime, } = req.body;
        // ---- Validate payPeriodType ----
        if (payPeriodType !== undefined &&
            !VALID_PAY_PERIOD_TYPES.includes(payPeriodType)) {
 ...
 * =====================================================
 */
router.get("/preview", verifyToken_1.verifyToken, async (req, res) => {
    try {
        const { organizationId } = req.user;
        ...
        let location = null;
        if (locationId) {
            location = await prisma_1.prisma.location.findFirst({
                where: {
        ...
        const settings = (0, payperiod_engine_1.buildEffectiveSettingsFromOrgAndLocation)({
            organization,
            location,
        });
        ...
        const period = (0, payperiod_engine_1.getPayPeriodRange)(settings, targetDate);
        return res.json({
            organizationId,
