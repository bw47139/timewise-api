import { PrismaClient, PayrollPeriod } from "@prisma/client";
...
export async function generatePayrollSummary(
  ...
  const employeeRows = employees.map((emp) => {
    ...
    const overtimeHours = Math.max(
      totalHours - WEEKLY_OT_THRESHOLD,
      0
    );
...
function computeTotalHours(
  ...
  for (const p of punches) {
    ...
    if (p.type === "OUT" && lastIn) {
      const diffMs =
        p.timestamp.getTime() - lastIn.getTime();

      if (diffMs > 0) {
        const hours = diffMs / 1000 / 60 / 60;
        map.set(
          p.employeeId,
          (map.get(p.employeeId) ?? 0) + hours
        );
      }
      lastIn = null;
    }
...
function computeDailyHours(
  ...
  for (const [empId, dayMap] of map.entries()) {
    result.set(
      empId,
      Array.from(dayMap.entries()).map(([date, hours]) => ({
        date,
        hours: round2(hours),
      }))
    );
